...
def main():
    ...
    print(f"Scanning {len(files)} audio files with {args.threads} threads...")
    entries: List[FingerprintEntry] = []
    with futures.ThreadPoolExecutor(max_workers=args.threads) as ex:
        for res in tqdm(ex.map(scan_file, files), total=len(files), desc="Fingerprinting & Analyzing", unit="file"):
    ...
    total_dupes = sum(len(v) - 1 for v in dup_sets)
    total_space = sum(m.size for metas in dup_sets for m in metas) - sum(best_candidate(metas).size for metas in dup_sets)

    print(f"\nFound {len(dup_sets)} duplicate groups, {total_dupes} duplicate files.")
    print(f"Potential space to free: {total_space / (1024*1024):.2f} MB\n")
    ...
    for dup, _ in tqdm(actions, desc="Removing Duplicates", unit="file"):
        if args.delete:
    ...
    print("\nDone!")
    print(f"Duplicates processed: {len(actions)}")
    print(f"Space freed (approx): {total_space / (1024*1024):.2f} MB")
